...
# load data
aoi = BacaData('data/klaten/ORTHOMOSAICFLI_ProjectRas.png').patchData(512, 1)
hil = BacaData('data/klaten/hillshade.png').patchData(512, 1)
seg = BacaData('data/klaten/segmented.png').patchData(512, 1)
msk = BacaData('data/klaten/segmented.png').patchData(512, 1)
...
size = (2, 1, 1, 512, 512, 1)

unpatch_img = predict.reshape(size)
print(unpatch_img.shape)

reconstructed_image = p.unpatchify(unpatch_img, (1024, 512, 1))
print(f'ukuran akhir prediksi {reconstructed_image.shape}')

out_img = reconstructed_image[:700, :287, :]
...
cv2.imwrite('out/newdata_001.png', out_img)
